...
public class SRToolImpl implements SRTool
{
    ...
    public SRToolResult go() throws IOException, InterruptedException
    {
        ...
        {
            program = (Program) new LoopUnwinderVisitor(clArgs.unsoundBmc, clArgs.unwindDepth).visit(program);
        }
        else
        {
        ...
        if (queryResult.startsWith("unsat")) { return SRToolResult.CORRECT; }

        if (queryResult.startsWith("sat")) { return SRToolResult.INCORRECT; }
